...
import { ALLOCATION_NETWORK, ALLOCATION_SEASON } from '../server/persistence/allocationRepository';
...
test('12: second check returns the exact same allocation', async () => {
  ...
  assert.equal((await repo.findByWallet(WALLET, SEASON, NETWORK))?.allocation, 18_500);
  await repo.close();
});
...
test('19b: exhausted Season 01 pool returns pool_exhausted and writes nothing', async () => {
  const repo = new SqliteAllocationRepository(':memory:');
  ...
  assert.equal(await repo.countAllocations(SEASON, NETWORK), 1);
  assert.equal(await repo.totalAllocated(SEASON, NETWORK), 1000);
  await repo.close();
});
...
test('persistence survives repository restart (on-disk database)', async () => {
  ...
  try {
    ...
    const reloaded = await second.findByWallet(WALLET, SEASON, NETWORK);
